...
if __name__ == '__main__':
    ...
    for i in range(300):
        file_name = os.path.join(xml_path, f'00{str(1892 + i)}.xml')
        objects = parse_rec(file_name)
        out_file = os.path.join(out_path, f'00{str(1892 + i)}.txt')
        f = open(out_file, 'a') if os.path.isfile(out_file) else open(out_file, 'w')
        for i in range(len(objects)):
            bbox = ''.join(
                str(objects[i]['bbox'][j]) + ' '
